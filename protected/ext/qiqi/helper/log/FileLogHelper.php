...
class FileLogHelper extends Component
{
    ...
    public static function xlog($message, $trace = null, $type = '')
    {
        ...
        $message = sprintf("[%s]%s", date('Y-m-d H:i:s'), var_export($message, true));
    ...
    public static function syslog($message, $category = 'default', $type = '')
    {
        ...
        if(!is_string($message)){
            if(is_array($message)){
                ...
                //                $ret = $message->getAttributes();
                $ret = var_export($message, true);
            }
        }
        $ret .= "\n";
        $ret = sprintf("[%s]", date("H:i:s")) . $ret;
        try{
    ...
    public static function shell($message, $status = '')
    {
        ...
        echo sprintf("[%s]%s%s%s%s[0m\n", date("Y-m-d H:i:s"), chr(27), $out, var_export($message, true),
            chr(27));
